...
class Actions{
    ...
    async sticker(){
        if(!this.isImage){
            await this.bot.sendMessage(this.remoteJid, { text: `${BOT_EMOJI} ❌ Erro! É necessário mandar uma Imagem`})
            return
        }
    ...
    const outputPath = path.resolve(TEMP_FOLDER, 'output.webp')
        ...
        exec(`ffmpeg -i ${inputPath} -vf scale=512:512 ${outputPath}`, async (error) =>{
            if(error){
                await this.bot.sendMessage(this.remoteJid, { text: `${BOT_EMOJI} ❌ Erro ao converter a Imagem para Sticker`})
                return
            }
